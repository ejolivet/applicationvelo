...
def test_repository_can_save_a_component(session):
    compoenent = model_component.Component(
        identifiant=model_component.ComponentID("100"),
    ...
    rows = session.execute(
        'SELECT identifiant, component_name, component_type, parent_component_id FROM "tb_components"'
    )
    assert list(rows) == [("100", "Component1", "Type1", None)]
...
def test_repository_can_save_a_component_with_parent(session):
    ...
    assert list(rows) == [("100", "Component1", "Type1", "200")]
